...
getgenv().Parts = {"Head", "HumanoidRootPart, UpperTorso","LeftHand","RightHand","LeftFoot","UpperTorso","RightFoot"} -- {"Head", "HumanoidRootPart"}
...
getgenv().MacroKey = "q" -- to use the built in lua macro
...
task.spawn(function()
    while task.wait() do
        if getgenv().Settings.Resolver.UnderGround == true and Aiming.Selected ~= nil and Aiming.Selected.Character then
            local TargetVelv2 = Aiming.Selected.Character[getgenv().Parts[1]]
        ...
        if getgenv().Settings.Resolver.DetectDesync == true and Aiming.Selected ~= nil and Aiming.Selected.Character and Aiming.Selected.Character:WaitForChild("HumanoidRootPart").Velocity.magnitude > getgenv().Settings.Resolver.Detection then
            local TargetVel = Aiming.Selected.Character[getgenv().Parts[1]]
